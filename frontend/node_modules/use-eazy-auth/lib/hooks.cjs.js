...
function getResolvedOrPromise(value) {
  // Check if a Promise...
  if (value !== null && _typeof__default['default'](value) === 'object' && typeof value.then === 'function') {
    return value;
  }
  ...
  var getTokens = function getTokens() {
    return getResolvedOrPromise(storageBackend.getItem(storageNamespace)).then(function (rawTokens) {
      ...
      try {
        ...
        if (_typeof__default['default'](parsedTokens) === 'object' && parsedTokens !== null && parsedTokens.accessToken) {
...
function makeCallWithRefresh(accessToken, refreshToken, refreshTokenCall) {
  return function (apiFn) {
    return rxjs.from(apiFn(accessToken)).pipe(operators.map(function (response) {
      return {
        response: response,
      ...
      refreshToken && typeof refreshTokenCall === 'function') {
        return rxjs.from(refreshTokenCall(refreshToken)).pipe(operators.mergeMap(function (refreshedTokens) {
          // Yeah refresh appends!
          // Ok now retry the apiFn \w refreshed shit!
          return rxjs.from(apiFn(refreshedTokens.accessToken)).pipe(operators.map(function (response) {
...
function getBootAuthObservable(meCall, refreshTokenCall, storage) {
  return rxjs.from(storage.getTokens()).pipe(operators.mergeMap(function (tokensInStorage) {
    ...
    return callWithRefresh(meCall).pipe(operators.catchError(function (err) {
      // Clear bad tokens from storage
      storage.removeTokens();
      return rxjs.throwError(err);
    }), operators.map(function (responseWithRefresh) {
      return {
...
function bootAuth(meCall, refreshTokenCall, storage, dispatch, tokenRef, bootRef) {
  ...
  var subscription = getBootAuthObservable(meCall, refreshTokenCall, storage).subscribe(function (_ref) {
    ...
    endBoot(_objectSpread__default['default']({
      authenticated: true,
      user: user
...
function makePerformLogin(loginCall, meCall, storage, dispatch, tokenRef) {
  var loginTrigger = new rxjs.Subject();
  var subscription = loginTrigger.asObservable().pipe(operators.tap(function () {
    return dispatch({
      type: LOGIN_LOADING
    });
  }), operators.exhaustMap(function (loginCredentials) {
    return rxjs.from(loginCall(loginCredentials)).pipe(operators.mergeMap(function (loginResponse) {
      var accessToken = loginResponse.accessToken;
      return rxjs.from(meCall(accessToken, loginResponse)).pipe(operators.map(function (user) {
        return {
          type: LOGIN_SUCCESS,
          payload: {
...
function makeCallApiRx(refreshTokenCall, dispatch, storage, tokenRef, bootRef, actionObservable) {
  ...
  var logoutObservable = actionObservable.pipe(operators.filter(function (action) {
    return action.type === LOGOUT;
  })); // Subject for emit refresh tasks

  var refreshEmitter = new rxjs.Subject(); // An Observable that perform the refresh token call
  // until logout was dispatched and emit actions

  var refreshRoutine = refreshEmitter.asObservable().pipe(operators.exhaustMap(function (refreshToken) {
    return rxjs.concat(rxjs.of(tokenRefreshing()), rxjs.from(refreshTokenCall && refreshToken ? refreshTokenCall(refreshToken) : rxjs.throwError(null)).pipe(operators.map(function (refreshResponse) {
    ...
    }), operators.takeUntil(logoutObservable)));
  ...
  function waitForStoreRefreshObservable() {
    return actionObservable.pipe(operators.filter(function (action) {
  ...
  function getAccessToken() {
    ...
    if (!authBooted) {
      waitBootObservable = actionObservable.pipe(operators.filter(function (action) {
        return action.type === BOOTSTRAP_AUTH_END;
      }), operators.take(1), operators.mergeMap(function () {
        return rxjs.EMPTY;
      }));
    } else {
      waitBootObservable = rxjs.EMPTY;
    }

    return rxjs.concat(waitBootObservable, rxjs.defer(function () {
  ...
  function unauthLogout(badAccessToken, error) {
    ...
    if (accessToken !== null && !refreshingSemaphore && accessToken === badAccessToken) {
      if (_typeof__default['default'](error) === 'object' && error.status === 401) {
        logout();
      }
    ...
    if (firstAccessToken !== null) {
      ...
      if (error.status === 401) {
        ...
        return refreshOnUnauth(firstAccessToken).pipe(operators.mergeMap(function (accessToken) {
          return rxjs.from(apiFn(accessToken).apply(void 0, _toConsumableArray__default['default'](args))).pipe(operators.catchError(function (error) {
  ...
  var firstBootSub = actionObservable.pipe(operators.filter(function (action) {
    return action.type === BOOTSTRAP_AUTH_END;
  }), operators.take(1)).subscribe(function () {
    refreshRoutine.connect();
  });
  var logoutSub = actionObservable.pipe(operators.filter(function (action) {
    return action.type === LOGOUT;
  })).subscribe(function () {
...
function initAuthState(initialData) {
  if (initialData) {
    ...
    if (initialData.user && initialData.accessToken) {
      ...
      return _objectSpread__default['default'](_objectSpread__default['default']({}, initialState), {}, {
...
function authReducer() {
  ...
  switch (action.type) {
    case LOGIN_LOADING:
      return _objectSpread__default['default'](_objectSpread__default['default']({}, previousState), {}, {
        loginLoading: true,
        loginError: null
      });
    ...
    case BOOTSTRAP_AUTH_END:
      {
        var nextState = _objectSpread__default['default'](_objectSpread__default['default']({}, previousState), {}, {
          bootstrappedAuth: true,
          bootstrappingAuth: false
        });
    ...
    case PATCH_USER:
      return _objectSpread__default['default'](_objectSpread__default['default']({}, previousState), {}, {
        user: _objectSpread__default['default'](_objectSpread__default['default']({}, previousState.user), action.payload)
      });
...
var AuthStateContext = /*#__PURE__*/React.createContext(null);
var AuthUserContext = /*#__PURE__*/React.createContext(null);
var AuthActionsContext = /*#__PURE__*/React.createContext(null);
function Auth(_ref) {
  ...
  // Init React Reducer
  var _useReducer = React.useReducer(authReducer, initialData, initAuthState),
  ...
  var tokenRef = React.useRef(accessToken ? {
    accessToken: accessToken,
    refreshToken: refreshToken,
    expires: expires
  ...
  var bootRef = React.useRef(bootstrappedAuth);
  ...
  var _useConstant = useConstant(function () {
    ...
    return [actionSubject.asObservable(), dispatch];
      ...
      _useConstant2 = _slicedToArray__default['default'](_useConstant, 2),
  ...
  var _useConstant3 = useConstant(function () {
    return makePerformLogin(loginCall, meCall, storage, dispatch, tokenRef);
      ...
      _useConstant4 = _slicedToArray__default['default'](_useConstant3, 2),
      performLogin = _useConstant4[0],
      unsubscribeFromLogin = _useConstant4[1];

  var login = React.useCallback(function (loginCredentials) {
  ...
  var performLogout = React.useCallback(function () {
    // Trigger log out
    dispatch({
      type: LOGOUT
    });
  ...
  var setTokens = React.useCallback(function (tokensBag) {
  ...
  var actions = React.useMemo(function () {
    return _objectSpread__default['default'](_objectSpread__default['default']({}, boundActionCreators), {}, {
      callAuthApiPromise: callAuthApiPromise,
      callAuthApiObservable: callAuthApiObservable,
      login: login,
      logout: logout,
  ...
  var authState = React.useMemo(function () {
    return {
  ...
  var authUser = React.useMemo(function () {
    return {
      user: state.user,
      token: accessToken
    };
  ...
  return /*#__PURE__*/React__default['default'].createElement(AuthActionsContext.Provider, {
    value: actions
  }, /*#__PURE__*/React__default['default'].createElement(AuthStateContext.Provider, {
    value: authState
  }, /*#__PURE__*/React__default['default'].createElement(AuthUserContext.Provider, {
    value: authUser
  }, typeof render === 'function' ? render(actions, authState, authUser) : children)));
}

function useAuthState() {
  var authState = React.useContext(AuthStateContext);
  return authState;
}
function useAuthActions() {
  var actions = React.useContext(AuthActionsContext);
  return actions;
}
function useAuthUser() {
  var user = React.useContext(AuthUserContext);
  return user;
}
...
function useLogin() {
  ...
  var loginWithCredentials = React.useCallback(function () {
    login(credentials);
  }, [login, credentials]);
  ...
  var valuesProps = React.useMemo(function () {
    return credentialsConf.reduce(function (out, name) {
      return _objectSpread__default['default'](_objectSpread__default['default']({}, out), {}, _defineProperty__default['default']({}, name, {
        value: credentials[name] === undefined ? '' : credentials[name]
      }));
    }, {});
  }, [credentials, credentialsConf]);
  var onChangesProps = React.useMemo(function () {
    return credentialsConf.reduce(function (out, name) {
      return _objectSpread__default['default'](_objectSpread__default['default']({}, out), {}, _defineProperty__default['default']({}, name, {
        onChange: function onChange(e) {
          var value = e.target.value;
          setCredentials(function (credentials) {
            return _objectSpread__default['default'](_objectSpread__default['default']({}, credentials), {}, _defineProperty__default['default']({}, name, value));
          });
        }
      }));
    }, {});
  }, [setCredentials, credentialsConf]);
  var credentialsProps = React.useMemo(function () {
    return Object.keys(valuesProps).reduce(function (r, name) {
      return _objectSpread__default['default'](_objectSpread__default['default']({}, r), {}, _defineProperty__default['default']({}, name, _objectSpread__default['default'](_objectSpread__default['default']({}, valuesProps[name]), onChangesProps[name])));
    }, {});
